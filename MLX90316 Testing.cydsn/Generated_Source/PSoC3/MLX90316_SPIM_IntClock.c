/*******************************************************************************
* File Name: MLX90316_SPIM_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_Start
********************************************************************************
*
* Summary:
...
void MLX90316_SPIM_IntClock_Start(void) 
{
    ...
    MLX90316_SPIM_IntClock_CLKEN |= MLX90316_SPIM_IntClock_CLKEN_MASK;
	MLX90316_SPIM_IntClock_CLKSTBY |= MLX90316_SPIM_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_Stop
********************************************************************************
*
* Summary:
...
void MLX90316_SPIM_IntClock_Stop(void) 
{
    ...
    MLX90316_SPIM_IntClock_CLKEN &= (uint8)(~MLX90316_SPIM_IntClock_CLKEN_MASK);
	MLX90316_SPIM_IntClock_CLKSTBY &= (uint8)(~MLX90316_SPIM_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((MLX90316_SPIM_IntClock_CLKEN & MLX90316_SPIM_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = MLX90316_SPIM_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = MLX90316_SPIM_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* MLX90316_SPIM_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(MLX90316_SPIM_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(MLX90316_SPIM_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = MLX90316_SPIM_IntClock_GetSourceRegister();
    uint16 oldDivider = MLX90316_SPIM_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = MLX90316_SPIM_IntClock_CLKEN & MLX90316_SPIM_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(MLX90316_SPIM_IntClock_DIV_PTR, clkDivider);
                MLX90316_SPIM_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                MLX90316_SPIM_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(MLX90316_SPIM_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((MLX90316_SPIM_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(MLX90316_SPIM_IntClock_DIV_PTR, clkDivider);
				MLX90316_SPIM_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(MLX90316_SPIM_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    MLX90316_SPIM_IntClock_MOD_SRC |= modeBitMask & (uint8)MLX90316_SPIM_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    MLX90316_SPIM_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(MLX90316_SPIM_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return MLX90316_SPIM_IntClock_MOD_SRC & (uint8)(MLX90316_SPIM_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = MLX90316_SPIM_IntClock_GetDividerRegister();
    uint8 oldSrc = MLX90316_SPIM_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        MLX90316_SPIM_IntClock_MOD_SRC |= CYCLK_SSS;
        MLX90316_SPIM_IntClock_MOD_SRC =
            (MLX90316_SPIM_IntClock_MOD_SRC & (uint8)(~MLX90316_SPIM_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return MLX90316_SPIM_IntClock_MOD_SRC & MLX90316_SPIM_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    MLX90316_SPIM_IntClock_PHASE = clkPhase & MLX90316_SPIM_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: MLX90316_SPIM_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return MLX90316_SPIM_IntClock_PHASE & MLX90316_SPIM_IntClock_PHASE_MASK;
}
